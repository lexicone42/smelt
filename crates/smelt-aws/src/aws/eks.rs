...
impl AwsProvider {
    pub(super) fn eks_cluster_schema() -> smelt_provider::ResourceTypeInfo {
    ...
    pub(super) async fn read_eks_cluster(
        ...
        if let Some(val) = resource.version().filter(|s| !s.is_empty()) {
            if state.get("sizing").is_none() {
                state["sizing"] = serde_json::json!({});
            }
            state["sizing"]["version"] = serde_json::json!(val);
        }
    ...
    pub(super) fn eks_node_group_schema() -> smelt_provider::ResourceTypeInfo {
        smelt_provider::ResourceTypeInfo {
