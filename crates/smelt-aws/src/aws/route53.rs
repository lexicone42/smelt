// Generated by smelt-codegen batch (aws) — do not edit by hand.
// Service: route53
// Resources: 2

use smelt_provider::*;
use std::collections::HashMap;

use super::AwsProvider;

impl AwsProvider {
    pub(super) fn route53_hosted_zone_schema() -> smelt_provider::ResourceTypeInfo {
        smelt_provider::ResourceTypeInfo {
            type_path: "route53.HostedZone".into(),
            description: "HostedZone resource".into(),
            schema: smelt_provider::ResourceSchema {
                sections: vec![smelt_provider::SectionSchema {
                    name: "identity".into(),
                    description: "Identity configuration".into(),
                    fields: vec![smelt_provider::FieldSchema {
                        name: "name".into(),
                        description: "Domain name (e.g., example.com)".into(),
                        field_type: smelt_provider::FieldType::String,
                        required: true,
                        default: None,
                        sensitive: false,
                    }],
                }],
            },
        }
    }

    pub(super) async fn create_route53_hosted_zone(
        &self,
        config: &serde_json::Value,
    ) -> Result<ResourceOutput, ProviderError> {
        // Extract fields from config
        let name = config.require_str("/identity/name")?.to_string();

        let mut req = self.route53_client.create_hosted_zone().name(&name);

        let caller_ref = format!("smelt-{}", chrono::Utc::now().timestamp());
        req = req.caller_reference(&caller_ref);

        let result = req
            .send()
            .await
            .map_err(|e| ProviderError::ApiError(format!("create_hosted_zone: {e}")))?;

        let container = result.hosted_zone().ok_or_else(|| {
            ProviderError::ApiError("create_hosted_zone returned no hosted_zone".into())
        })?;
        let provider_id = container.id();

        let provider_id = provider_id.trim_start_matches("/hostedzone/");
        self.read_route53_hosted_zone(provider_id).await
    }

    pub(super) async fn read_route53_hosted_zone(
        &self,
        provider_id: &str,
    ) -> Result<ResourceOutput, ProviderError> {
        let result = self
            .route53_client
            .get_hosted_zone()
            .id(provider_id)
            .send()
            .await
            .map_err(|e| ProviderError::ApiError(format!("get_hosted_zone: {e}")))?;

        let resource = result
            .hosted_zone()
            .ok_or_else(|| ProviderError::NotFound(format!("HostedZone {provider_id}")))?;

        let state = serde_json::json!({
            "identity": {
                "name": resource.name(),
            },
        });

        let mut outputs = HashMap::new();
        outputs.insert("hosted_zone_id".into(), serde_json::json!(provider_id));
        outputs.insert("name".into(), serde_json::json!(resource.name()));

        Ok(ResourceOutput {
            provider_id: provider_id.to_string(),
            state,
            outputs,
        })
    }

    #[allow(dead_code)]
    pub(super) async fn update_route53_hosted_zone(
        &self,
        _provider_id: &str,
        _config: &serde_json::Value,
    ) -> Result<ResourceOutput, ProviderError> {
        Err(ProviderError::RequiresReplacement(
            "resource does not support in-place update".into(),
        ))
    }

    pub(super) async fn delete_route53_hosted_zone(
        &self,
        provider_id: &str,
    ) -> Result<(), ProviderError> {
        self.route53_client
            .delete_hosted_zone()
            .id(provider_id)
            .send()
            .await
            .map_err(|e| ProviderError::ApiError(format!("delete_hosted_zone: {e}")))?;
        Ok(())
    }

    pub(super) fn route53_record_set_schema() -> smelt_provider::ResourceTypeInfo {
        smelt_provider::ResourceTypeInfo {
            type_path: "route53.RecordSet".into(),
            description: "RecordSet resource".into(),
            schema: smelt_provider::ResourceSchema {
                sections: vec![
                    smelt_provider::SectionSchema {
                        name: "identity".into(),
                        description: "Identity configuration".into(),
                        fields: vec![smelt_provider::FieldSchema {
                            name: "name".into(),
                            description: "Record name (FQDN)".into(),
                            field_type: smelt_provider::FieldType::String,
                            required: true,
                            default: None,
                            sensitive: false,
                        }],
                    },
                    smelt_provider::SectionSchema {
                        name: "network".into(),
                        description: "Network configuration".into(),
                        fields: vec![
                            smelt_provider::FieldSchema {
                                name: "hosted_zone_id".into(),
                                description: "Hosted zone ID".into(),
                                field_type: smelt_provider::FieldType::String,
                                required: true,
                                default: None,
                                sensitive: false,
                            },
                            smelt_provider::FieldSchema {
                                name: "record_type".into(),
                                description: "Record type".into(),
                                field_type: smelt_provider::FieldType::String, /* Enum */
                                required: true,
                                default: None,
                                sensitive: false,
                            },
                            smelt_provider::FieldSchema {
                                name: "ttl".into(),
                                description: "Time to live in seconds".into(),
                                field_type: smelt_provider::FieldType::Integer,
                                required: false,
                                default: Some(serde_json::json!(300)),
                                sensitive: false,
                            },
                            smelt_provider::FieldSchema {
                                name: "values".into(),
                                description: "Record values".into(),
                                field_type: smelt_provider::FieldType::Array(Box::new(
                                    smelt_provider::FieldType::String,
                                )),
                                required: false,
                                default: None,
                                sensitive: false,
                            },
                        ],
                    },
                ],
            },
        }
    }

    pub(super) async fn create_route53_record_set(
        &self,
        config: &serde_json::Value,
    ) -> Result<ResourceOutput, ProviderError> {
        let hosted_zone_id = config
            .get("hosted_zone_id")
            .or_else(|| config.pointer("/network/hosted_zone_id"))
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                ProviderError::InvalidConfig("hosted_zone_id is required for RecordSet".into())
            })?;

        let record_name = config
            .pointer("/network/name")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ProviderError::InvalidConfig("network.name is required".into()))?;

        let record_type = config
            .pointer("/network/record_type")
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                ProviderError::InvalidConfig("network.record_type is required".into())
            })?;

        let ttl = config
            .pointer("/network/ttl")
            .and_then(|v| v.as_i64())
            .unwrap_or(300);

        let mut rr_set = aws_sdk_route53::types::ResourceRecordSet::builder()
            .name(record_name)
            .r#type(aws_sdk_route53::types::RrType::from(record_type))
            .ttl(ttl);

        // Alias record (e.g., pointing to ALB)
        if let Some(alias_target) = config.pointer("/network/alias") {
            let dns_name = alias_target
                .get("dns_name")
                .and_then(|v| v.as_str())
                .unwrap_or("");
            let hz_id = alias_target
                .get("hosted_zone_id")
                .and_then(|v| v.as_str())
                .unwrap_or("");

            rr_set = rr_set
                .alias_target(
                    aws_sdk_route53::types::AliasTarget::builder()
                        .dns_name(dns_name)
                        .hosted_zone_id(hz_id)
                        .evaluate_target_health(true)
                        .build()
                        .map_err(|e| {
                            ProviderError::InvalidConfig(format!(
                                "failed to build AliasTarget: {e}"
                            ))
                        })?,
                )
                .set_ttl(None); // Alias records don't have TTL
        } else {
            // Standard record values
            if let Some(values) = config.pointer("/network/values").and_then(|v| v.as_array()) {
                for val in values {
                    if let Some(v) = val.as_str() {
                        rr_set = rr_set.resource_records(
                            aws_sdk_route53::types::ResourceRecord::builder()
                                .value(v)
                                .build()
                                .map_err(|e| {
                                    ProviderError::InvalidConfig(format!(
                                        "failed to build ResourceRecord: {e}"
                                    ))
                                })?,
                        );
                    }
                }
            }
        }

        let change = aws_sdk_route53::types::Change::builder()
            .action(aws_sdk_route53::types::ChangeAction::Upsert)
            .resource_record_set(rr_set.build().map_err(|e| {
                ProviderError::InvalidConfig(format!("failed to build ResourceRecordSet: {e}"))
            })?)
            .build()
            .map_err(|e| ProviderError::InvalidConfig(format!("failed to build Change: {e}")))?;

        self.route53_client
            .change_resource_record_sets()
            .hosted_zone_id(hosted_zone_id)
            .change_batch(
                aws_sdk_route53::types::ChangeBatch::builder()
                    .changes(change)
                    .build()
                    .map_err(|e| {
                        ProviderError::InvalidConfig(format!("failed to build ChangeBatch: {e}"))
                    })?,
            )
            .send()
            .await
            .map_err(|e| ProviderError::ApiError(format!("ChangeResourceRecordSets: {e}")))?;

        let provider_id_str = format!("{hosted_zone_id}:{record_name}:{record_type}");
        let provider_id = provider_id_str.as_str();

        self.read_route53_record_set(provider_id).await
    }

    pub(super) async fn read_route53_record_set(
        &self,
        provider_id: &str,
    ) -> Result<ResourceOutput, ProviderError> {
        let parts: Vec<&str> = provider_id.splitn(3, ':').collect();
        if parts.len() != 3 {
            return Err(ProviderError::InvalidConfig(
                "RecordSet provider_id must be zone_id:name:type".into(),
            ));
        }
        let (zone_id, record_name, record_type) = (parts[0], parts[1], parts[2]);

        let result = self
            .route53_client
            .list_resource_record_sets()
            .hosted_zone_id(zone_id)
            .start_record_name(record_name)
            .start_record_type(aws_sdk_route53::types::RrType::from(record_type))
            .max_items(1)
            .send()
            .await
            .map_err(|e| ProviderError::ApiError(format!("ListResourceRecordSets: {e}")))?;

        let rr = result
            .resource_record_sets()
            .first()
            .ok_or_else(|| ProviderError::NotFound(format!("RecordSet {provider_id}")))?;

        let values: Vec<String> = rr
            .resource_records()
            .iter()
            .map(|r| r.value().to_string())
            .collect();

        let state = serde_json::json!({
            "network": {
                "name": rr.name(),
                "record_type": rr.r#type().as_str(),
                "ttl": rr.ttl().unwrap_or(0),
                "values": values,
            }
        });

        let mut outputs = HashMap::new();
        outputs.insert("fqdn".into(), serde_json::json!(rr.name()));

        Ok(ResourceOutput {
            provider_id: provider_id.to_string(),
            state,
            outputs,
        })
    }

    pub(super) async fn update_route53_record_set(
        &self,
        provider_id: &str,
        config: &serde_json::Value,
    ) -> Result<ResourceOutput, ProviderError> {
        // Upsert handles both create and update — delegate to create which does UPSERT
        let parts: Vec<&str> = provider_id.splitn(3, ':').collect();
        if parts.len() != 3 {
            return Err(ProviderError::InvalidConfig(
                "RecordSet provider_id must be zone_id:name:type".into(),
            ));
        }
        let zone_id = parts[0];

        let mut config_with_zone = config.clone();
        config_with_zone["hosted_zone_id"] = serde_json::json!(zone_id);
        // create_route53_record_set does UPSERT + read, codegen also adds read, so suppress
        let _ = self.create_route53_record_set(&config_with_zone).await?;

        self.read_route53_record_set(provider_id).await
    }

    pub(super) async fn delete_route53_record_set(
        &self,
        provider_id: &str,
    ) -> Result<(), ProviderError> {
        let parts: Vec<&str> = provider_id.splitn(3, ':').collect();
        if parts.len() != 3 {
            return Err(ProviderError::InvalidConfig(
                "RecordSet provider_id must be zone_id:name:type".into(),
            ));
        }
        let (zone_id, record_name, record_type) = (parts[0], parts[1], parts[2]);

        // Get current record to delete it
        let current = self.read_route53_record_set(provider_id).await?;
        let ttl = current
            .state
            .pointer("/network/ttl")
            .and_then(|v| v.as_i64())
            .unwrap_or(300);
        let values: Vec<&str> = current
            .state
            .pointer("/network/values")
            .and_then(|v| v.as_array())
            .map(|a| a.iter().filter_map(|v| v.as_str()).collect())
            .unwrap_or_default();

        let mut rr_set = aws_sdk_route53::types::ResourceRecordSet::builder()
            .name(record_name)
            .r#type(aws_sdk_route53::types::RrType::from(record_type))
            .ttl(ttl);

        for v in &values {
            rr_set = rr_set.resource_records(
                aws_sdk_route53::types::ResourceRecord::builder()
                    .value(*v)
                    .build()
                    .map_err(|e| {
                        ProviderError::InvalidConfig(format!("failed to build ResourceRecord: {e}"))
                    })?,
            );
        }

        let change = aws_sdk_route53::types::Change::builder()
            .action(aws_sdk_route53::types::ChangeAction::Delete)
            .resource_record_set(rr_set.build().map_err(|e| {
                ProviderError::InvalidConfig(format!("failed to build ResourceRecordSet: {e}"))
            })?)
            .build()
            .map_err(|e| ProviderError::InvalidConfig(format!("failed to build Change: {e}")))?;

        self.route53_client
            .change_resource_record_sets()
            .hosted_zone_id(zone_id)
            .change_batch(
                aws_sdk_route53::types::ChangeBatch::builder()
                    .changes(change)
                    .build()
                    .map_err(|e| {
                        ProviderError::InvalidConfig(format!("failed to build ChangeBatch: {e}"))
                    })?,
            )
            .send()
            .await
            .map_err(|e| {
                ProviderError::ApiError(format!("ChangeResourceRecordSets (DELETE): {e}"))
            })?;
        Ok(())
    }
}
